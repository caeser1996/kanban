...
const MultiKanbanBoard: React.FC = () => {
  ...
  const openModal = (boardName: string, columnId: string, task?: Task) => {
    ...
    if (task) {
      ...
      setNewTask({
        ...
        title: "",
        state:
          boardStates[boardName].find((col) => col.id === columnId)?.title ||
          "",
        assignee: "",
        tags: [],
        priority: "Medium",
  ...
  const getCardBorderColor = (stage: string) => {
    const colors = ['border-blue-400', 'border-green-400', 'border-yellow-400', 'border-red-400', 'border-purple-400', 'border-indigo-400'];
    const index = stageConfig.findIndex(s => s.id === stage);
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      <div className="flex border-b bg-white">
        <div className="w-1/6 p-2 font-semibold text-gray-600"></div>
        {stageConfig.map((stage) => (
          <div
            key={stage.id}
            className="flex-1 text-center p-2 font-semibold text-gray-600 border border-gray-300 rounded-md m-1" // Added border and margin
          >
            {stage.title}
          </div>
        ))}
      ...
      <div className="flex-1 overflow-auto bg-gray-200">
        {boardNames.map((boardName, boardIndex) => (
          <div key={boardIndex} className="mb-4">
            ...
            <div className="flex">
              {/* Work Item column */}
              <div className="w-1/6 bg-white flex flex-col items-center justify-start h-full p-4">
                <div className="bg-purple-100 text-purple-700 p-2 rounded-lg text-center w-full border border-purple-300">
                  {boardName}
                </div>
                <button
                  onClick={() => openModal(boardName, "backlog")}
                  className="w-full text-left p-2 mt-4 bg-gray-100 hover:bg-gray-200 rounded flex items-center justify-center"
                >
              ...
              <div className="flex-1 flex bg-gray-100">
                <DragDropContext
                  onDragEnd={(result) => onDragEnd(result, boardName)}
                >
                  {boardStates[boardName]?.map((column, columnIndex) => (
                    <div
                      key={column.id}
                      className="flex-1 px-2 p-2 m-1 rounded border border-gray-400"
                    >
                              ...
                              {column.tasks.map((task, taskIndex) => (
                                ...
                                >
                                  {(provided) => (
                                    ...
                                    //   className="p-2 mb-2 bg-white ${getCardBorderColor(stage.id)} rounded shadow-sm hover:shadow group"
                                    className={`mb-2 bg-white rounded shadow-sm ${getCardBorderColor(column.id)} border-l-4 p-2 group`}
                                    >
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="bg-white p-5 rounded-lg w-96">
            ...
            <input
              ...
              onChange={(e) =>
                setNewTask({ ...newTask, tags: e.target.value.split(", ") })
              }
            ...
            <input
              ...
              onChange={(e) =>
                setNewTask({ ...newTask, areaPath: e.target.value })
              }
            ...
            <input
              ...
              onChange={(e) =>
                setNewTask({
                  ...newTask,
                  effort: parseInt(e.target.value) || 0,
                })
              }
            ...
            <input
              ...
              onChange={(e) =>
                setNewTask({ ...newTask, iteration: e.target.value })
              }
