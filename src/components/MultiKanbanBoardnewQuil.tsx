...
const MultiKanbanBoard: React.FC = () => {
  ...
  const openModal = (boardName: string, task?: Task) => {
    ...
    if (task) {
      setEditingTask(task);
      setNewTask({ ...task, activities: task.activities || [] });
    } else {
      setEditingTask(null);
      setNewTask({
        ...
        <div className="flex-1 overflow-auto">
          {Object.entries(filteredBoardStates).map(([boardName, columns], boardIndex) => (
            <div key={boardName} className="mb-4">
              ...
              {expandedBoards.includes(boardName) && (
                <div className="flex mt-2" style={{ height: '400px' }}> {/* Set a fixed height for the board */}
                  {columns.map((column) => (
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="bg-white p-5 rounded-lg w-3/4 max-w-4xl">
            ...
            {editingTask && (
              <div className="mb-4">
                <button
                  ...
                  className={`mr-2 p-2 ${activeTab === "main" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
                >
                  Main
                </button>
                <button
                  onClick={() => setActiveTab("activity")}
                  className={`p-2 ${activeTab === "activity" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
                >
                  Activity Log
                </button>
              </div>
            )}
            {(activeTab === "main" || !editingTask) && (
              <div>
                <input
                  type="text"
                  value={newTask.title}
                ...
                <input
                  ...
                  className="w-full p-2 mb-2 border rounded"
                />
                <div style={{ height: '200px' }}> {/* Set a fixed height for the Quill editor container */}
                  <ReactQuill
            ...
            <div className="flex justify-end mt-4">
              <button
                onClick={closeModal}
                className="bg-purple-100 text-purple-700 p-2 rounded flex items-center rounded mr-2"
              >
                Cancel
              </button>
              <button
